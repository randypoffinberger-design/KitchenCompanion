...
The recipe remains installed and can be restored from Settings → Hidden Recipes.`)) return;
  ...
  function setImageImportStage(stage) {
    ...
    if (!ready) {
      ...
      const rawOcr=document.querySelector('#rawRecognizedRecipeText'), rawDetails=document.querySelector('#rawOcrDetails');
      if(rawOcr)rawOcr.value=''; if(rawDetails){rawDetails.hidden=true;rawDetails.open=false;}
  ...
  init();
  SKPasswordUI.init({sync:householdSync,renderAccount:renderCloudAccount,openAccount:openCloudAccount,resetServer:"https://api.serenityvalleyworks.com/sk"});
})();
